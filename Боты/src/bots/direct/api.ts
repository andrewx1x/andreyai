...
async function directRequest<T>(
  ...
  try {
    ...
    // Check for report status (202 = pending)
    if (response.status === 202) {
      // Report is being generated, retry after delay
    ...
    if (!response.ok) {
      const errorData = (await response.json()) as DirectApiError;
      return {
        ok: false,
...
export async function validateTokenAndLogin(
  ...
  const result = await directRequest<CampaignsResponse>('campaigns', token, login, {
    method: 'get',
    params: {
      SelectionCriteria: {},
...
export async function getStats(
    ...
    selectionCriteria['Filter'] = [
      {
        Field: 'CampaignId',
        Operator: 'IN',
        Values: params.campaignIds.map(String),
      },
    ];
  }
  ...
  const result = await directRequest<string>(
    'reports',
    token,
    login,
    {
  ...
  // Parse TSV response
  const stats = parseTsvReport(result.data as string, params.includeConversions);
...
export async function getCampaignStats(
  ...
  const reportName = `CampaignReport_${Date.now()}`;
  ...
  // Parse TSV response
  const stats = parseCampaignTsvReport(result.data as string, params.includeConversions);
  return { ok: true, data: stats };
}
...
function parseCampaignTsvReport(
  tsv: string,
...
export function getErrorMessage(errorCode?: number): string {
  switch (errorCode) {
    ...
    case 53:
      return 'Нет доступа к этому аккаунту. Проверьте логин.';
    case 54:
      return 'Слишком много запросов. Попробуйте через минуту.';
    case 500:
    case 502:
    case 503:
      return 'Сервер Яндекс.Директа временно недоступен. Попробуйте позже.';
    default:
